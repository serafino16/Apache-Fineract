...
resource "aws_waf_rule" "xss" {
  for_each    = { for k, v in var.waf_environments : k => v if v.enable }
  provider    = aws.region_providers[each.value.region]
  name        = "XSSRule-${each.key}"
  metric_name = "XSSMetric_${each.key}"

  predicates {
    data_id = aws_waf_xss_match_set.xss_match[each.key].id
    negated = false
    type    = "XssMatch"
  }
}

resource "aws_waf_xss_match_set" "xss_match" {
  for_each = { for k, v in var.waf_environments : k => v if v.enable }
  provider = aws.region_providers[each.value.region]
  name     = "XSSMatchSet_${each.key}"
